...
- Last rows: hold proof + hard cut to the next scene. **Last scene:** the final row is end of video at `master_duration_s` (no cut row, no scene N+1), and the final-frame proof must land *at or before* that moment — the tail after the last word can be very short (100ms in the first video), so pick the proof timestamp inside the tail, not a generic offset past it.
