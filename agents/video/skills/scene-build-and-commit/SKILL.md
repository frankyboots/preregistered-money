...
**Circular/stamp fit: constrain to the innermost ring, at the element's depth.** When an element must sit *inside* a circle (a seal, badge, or ringed mark), the binding constraint is the chord of the innermost ring it visually sits inside, evaluated at the element's vertical depth from center — `chord = 2·√(r² − dy²)`, r = that ring's centerline radius, dy = the element's offset below/above center. A "fits" estimate taken against the *outer* ring overstates the usable width and can leave the element crossing the inner ring, which then reads as not belonging to the stamp. If a spec already bakes in an inner-ring crossing as "accepted" on the strength of the outer-ring chord, that is a spec defect — recompute against the inner ring at the element's actual dy and fix the size (or reposition) rather than accepting the crossing; the fix rides with the build as a measured-geometry amendment (Step 1).
