...
description: "Use when a rough video request needs specs first."
version: 1.0.0
...
metadata:
  hermes:
    tags: [video, intake, storyboard, spec, planning]
    editorial_name: Video Intake And Storyboard
    editorial_description: "Turns a conversational video request into a spec another agent can execute without guessing: scene specs, beat timing, approval boundaries, and timestamp semantics."
---
...
## Intake Artifact (this repo)

The intake artifact is the video's `spec.md` — committed before the first render (VIDEO_CONCEPT §4.1): meta, beat sheet, number citations, inputs with checksums, standing items. Per-beat detail uses `videos/pipeline/templates/scene-spec.md`; multi-scene planning uses `videos/pipeline/templates/storyboard.md`. Every figure that will appear on screen gets a citation in the spec (results doc, or prereg/scoreboard for recaps) — an uncited number means the video requests a new prereg, it never computes one.
...
2. Extract style constraints.
   - Desired aesthetic (or the pinned style-spec version).
...
4. Identify missing decisions.
   ...
   - Prefer concrete choices: caption delivery, transition direction, final-frame priority, music question (standing answer: none).

## Scene Spec Rules

This skill owns the scene-spec *policy and quality bar*. Per-scene Phase B *execution* — window derivation, cue-table verification, change floor, the one-scene-per-commit loop — is `scene-spec-and-commit`.

- Use `videos/pipeline/templates/scene-spec.md` for each scene.
...
- For a channel's first video, the spec-SHA pin (`data/spec-sha.txt`) is a Phase A step — see `video-bootstrap`, step 2.

## Storyboard Rules

- Use `videos/pipeline/templates/storyboard.md` for multi-scene work.
...
- Reusable pattern: camera swipe, chart draw, caption preview, hold-frame pacing, final-frame proof.

Only reusable patterns belong in future skills or examples. Project content belongs in the video's spec and data inputs, cited.
...
- A number shown on screen that has no citation in the spec.

## Handoff Format

For implementation, hand off:

- Filled spec.md (or scene spec + storyboard when multi-scene).
- Asset list.
- Locked scene list.
- Open questions.
